...
public class BleManager {
    ...
    /*
    BluetoothAdapter類常用的方法說明：
    getDefaultAdapter : 獲取預設的藍芽介面卡。該方法為靜態方法。
    getState : 獲取藍芽的開關狀態。STATE_ON表示已開啟，STATE_TURNING_ON表示正在開啟，STATE_OFF表示已關閉，STATE_TURNING_OFF表示正在關閉。
    enable : 啟用藍芽功能。
    disable : 禁用藍芽功能。
    isEnabled : 判斷藍芽功能是否啟用。返回true表示已啟用，返回false表示未啟用。
    getBondedDevices : 獲取已配對的裝置集合。
    getRemoteDevice : 根據裝置地址獲取遠端的裝置物件。
    startDiscovery : 開始搜尋周圍的藍芽裝置。
    cancelDiscovery : 取消搜尋周圍的藍芽裝置。
    isDiscovering : 判斷是否正在搜尋周圍的藍芽裝置。
     */
    private MultipleBluetoothController multipleBluetoothController;
    private BluetoothManager bluetoothManager;   //藍芽管理器
    ...
    private static final int DEFAULT_WRITE_DATA_SPLIT_COUNT = 244;
    private static final int DEFAULT_CONNECT_OVER_TIME = 10000;
    private static final int DEFAULT_INTERVAL_BETWEEN_TWOPACKAGE = 0;
    ...
    private long interval_Between_TwoPackage = DEFAULT_INTERVAL_BETWEEN_TWOPACKAGE;
    ...
    public long getInterval_Between_TwoPackage() {
        return interval_Between_TwoPackage;
    }

    public BleManager setInterval_Between_TwoPackage(long num) {
        if (num >= 0) {
            this.interval_Between_TwoPackage = num;
        }
        return this;
    }
    ...
    public BluetoothGatt connect(BleDevice bleDevice, BleGattCallback bleGattCallback) {
        ...
        if (bleDevice == null || bleDevice.getDevice() == null) {
            ...
            //將新的Device建立至 bleTempHashMap(正在連接) 資料庫
    ...
    public void write(BleDevice bleDevice,
        ...
        //write(bleDevice, uuid_service, uuid_write, data, true, callback);//split 接割字串
        write(bleDevice, uuid_service, uuid_write, data, true, callback);   //split 接割字串
    }
    ...
    public void write(BleDevice bleDevice,
        ...
        write(bleDevice, uuid_service, uuid_write, data, split, true, getInterval_Between_TwoPackage(), callback);
    }
    ...
    public void setMtu(BleDevice bleDevice,
        ...
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            BleLog.e("API < 21 !");
            callback.onSetMTUFailure(new OtherException("API < 21"));
            return;
        }
     ...
     */
    /*
    public boolean requestLeConnectionUpdate(BleDevice bleDevice, int minConnectionInterval, int maxConnectionInterval,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P) {
            if (bleBluetooth == null) {
                ...
                return bleBluetooth.newBleConnector().requestLeConnectionUpdate(minConnectionInterval, maxConnectionInterval, slaveLatency, supervisionTimeout, minConnectionEventLen, maxConnectionEventLen);
            }
    ...
    */
     ...
     */
    public boolean setPreferredPhy(BleDevice bleDevice, int txPhy, int rxPhy, int phyOptions) {
        if (BluetoothAdapter.getDefaultAdapter().isLe2MPhySupported()
            ...
            if (bleBluetooth == null) {
                ...
                bleBluetooth.newBleConnector().setPreferredPhy(txPhy, rxPhy, phyOptions);
                return true;
            }
        }
        return false;
